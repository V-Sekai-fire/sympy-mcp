#!/usr/bin/env python3
"""
Test SymPy MCP server using fastmcp client with HTTP transport.
"""
...
async def test_http():
    """Test HTTP transport"""
    print("=" * 60)
    print("Testing SymPy MCP Server - HTTP Transport")
    print("=" * 60)
    
    http_url = "http://localhost:8081/"
    
    print(f"\nConnecting to HTTP endpoint: {http_url}\n")
    ...
    try:
        async with Client(http_url) as client:
            ...
            # Test sympy_evaluate
            print("7. Testing sympy_evaluate (x² + 2x + 1 with x=3)...")
            result = await client.call_tool("sympy_evaluate", {
                "expression": "x**2 + 2*x + 1",
                "substitutions": {"x": 3}
            })
            print(f"   Result: {result.content[0].text if result.content else 'No result'}\n")
